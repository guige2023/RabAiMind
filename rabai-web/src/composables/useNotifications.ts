...
export function useNotifications() {
  ...
  const createReminder = async (params: {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        showToast('✅ 提醒已设置', `将在 ${params.review_date} 前提醒您`, 'success')
  ...
  const createSmartAlert = async (params: {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        showToast('✅ 智能提醒已创建', `内容更新后将自动提醒您`, 'success')
        return res.data.data
      }
    } catch (e: any) {
  ...
  const sendMention = async (params: {
    ...
    try {
      ...
      if (res.data.success) {
        showToast('📣 @提及已发送', `已通知 ${params.to_user}`, 'success')
  ...
  const createDeadline = async (params: {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        showToast('⏰ 截止日期已设置', `"${params.title}" 已加入倒计时`, 'success')
        return res.data.data
      }
    } catch (e: any) {
      showToast('❌ 设置失败', e.message, 'error')
  ...
  const subscribeDigest = async (params: {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        showToast('📧 周报订阅成功', `每周 ${WEEKDAYS[params.day_of_week ?? 1]} ${params.hour ?? 9}:00 发送`, 'success')
        return res.data.data
      }
    } catch (e: any) {
  ...
  const testDigestEmail = async () => {
    try {
      ...
      if (res.data.success) {
        showToast('📧 测试邮件已发送', '请检查您的收件箱', 'success')
      }
    } catch (e: any) {
      showToast('❌ 发送失败', e.message, 'error')
    }
